...
if __name__ == "__main__":
    ...
    game_input = st.text_input(
        "Enter the name of a game you like and we'll give you similar ones!",
        "Call of Duty: World at War",
    )
    recommandation = ""
    if st.button(" recommend!"):
        recommandation = recommend_video_games(
            game_input, data_display, vg_distance, vg_indices, vector_names
        )
        if isinstance(recommandation, str):
            ...
            recommandation["genres"] = recommandation.apply(
                lambda row: ", ".join([col for col in columns_genre if row[col]]),
                axis=1,
            )
