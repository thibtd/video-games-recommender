...
def features_engineering(df_2024):
    ...
    vectorizer_plot = TfidfVectorizer(max_features=10000, stop_words="english").fit(
        game_plots
    )
    ...
    categorical_columns = [
        name for name in df_2024.columns if df_2024[name].dtype == "O"
    ]
...
def tf_idf_names(df):
    game_names = df.name
    vectorizer_name = TfidfVectorizer(max_features=10000, stop_words="english").fit(
        game_names
    )
...
def VideoGameTitleRecommender(
    video_game_name, game_names, game_name_vectors, vectorizer_name
):
    """
...
def VideoGameRecommender(
    ...
    if video_game_idx.empty:
        ...
        closest_match_game_name = VideoGameTitleRecommender(
            video_game_name, game_names, game_name_vectors, vectorizer_name
        )
        ...
        out = recommended_video_game_list
